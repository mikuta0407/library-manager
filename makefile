...
GHNAME = github.com/mikuta0407/library-manager
...
.PHONY: build
build:
	go build -o $(DIR)/$(NAME) $(LDFLAGS) $(GOFILE)

.PHONY: linux-arm64
linux-arm64:
	$(eval GOOS := linux)
	$(eval GOARCH := arm64)
	GOOS=$(GOOS) GOARCH=$(GOARCH) CGO_ENABLED=1 go build -o $(DIR)/$(NAME)_$(GOOS)_$(GOARCH) $(LDFLAGS) $(GOFILE)
...
windows-386:
	$(eval GOOS := windows)
	$(eval GOARCH := 386)
	GOOS=$(GOOS) GOARCH=$(GOARCH) CGO_ENABLED=1 go build -o $(DIR)/$(NAME)_$(GOOS)_$(GOARCH).exe $(LDFLAGS) $(GOFILE)
